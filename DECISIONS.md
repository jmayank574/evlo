...
### D20. Arrive-by "departure window" anchored to now (2026-06-15)
- **Bug:** the displayed window was `latest_safe_departure − max(now, pickup_open)`. With the synthetic pickup window opening at an odd 11 PM PT (later than now), it reduced to `latest − pickup` (~6h50m on LD-1001) — anchored to the pickup time, not the real current time.
- **Decision (operator-confirmed):** the figure is the dispatcher's **runway = `latest_safe_departure − now`** — time from the real current moment until the last instant the truck can roll. On LD-1001: now 12:11 PM PT, latest 5:49 AM PT next day → **17h 38m**. Negative once `now` passes the latest departure → time-infeasible ("latest safe departure was X ago").
- **Feasibility still respects the pickup window** (`time_ok = latest ≥ max(now, pickup)`): you can't dispatch before the load is available nor into the past. Only the *displayed* runway is now-anchored; ranking is unaffected (monotonic in `latest_departure` either way).
- Surfaced as a real domain field `departure_window_min` (testable, single-sourced); API `departure_slack_min` likewise anchored to `now`. Pinned by `test_arrive_by_departure_window_anchored_to_now_not_pickup` and the past-deadline edge test. Also de-flaked the feasibility/API tests that used the real wall clock against fixed deadlines.

### D19. Whole app pinned to Pacific; depart-at input is PT (2026-06-15)
- **Bug:** the depart-at `datetime-local` input was defaulted from the UTC wall-clock and sent naive → the backend read it as UTC, while everything else rendered in PT. Result: a 6:00 AM "PT" entry was computed as 6:00 AM UTC, and the ~7h offset nearly cancelled the ~7h trip, so arrival looked like "departure + 10 min" and seemed not to flow through. It actually did — it was a single UTC↔PT split.
- **Fix:** the input is now interpreted as **Pacific**. `toPacificInput()` defaults the box to the pickup's PT wall-clock; `pacificInputToUtcIso()` converts the typed PT wall-clock to a real UTC instant before sending (DST-correct via `Intl`). Backend computes in UTC; the frontend renders everything in PT. Verified on LD-1001: 6:00 AM PT + 6h15m + 25m + 30m = 1:10 PM PT.
- **Decision:** pin the **entire app to Pacific ("PT")** for now and drop the TX→CT branch from D18 — simpler and unambiguous for a single demo region. **Per-route timezone (CT for TX lanes, IANA-from-coordinates generally) is a deliberate future follow-up.** There is intentionally no UTC display or input path anywhere.
- **Tests:** backend pins `arrival == departure + drive + charge + dwell`; a new frontend vitest suite (`format.test.ts`) pins PT-only labeled rendering (never UTC) and the PT↔UTC input round-trip.
